...
client_mem = dcc.Store(id='client-mem', storage_type='session', data=ClientMemModel([0,0,0,0,0,0]))

initial_conditions_btn = dbc.Button(id='ic-btn', children='New Initial Conditions', n_clicks=0)
...
function_dropdown = dcc.Dropdown(id='function-dropdown', options=[
...
def update_initial_conditions(client_mem):
    if client_mem is None:
        ...
        return client_mem.initial_conditions[0], client_mem.initial_conditions[1], client_mem.initial_conditions[2], client_mem.initial_conditions[3], client_mem.initial_conditions[4], client_mem.initial_conditions[5]


@app.callback(
    Output('output-gif-div', 'children'),
    [Input('initial-conditions-btn', 'n_clicks')]
)
